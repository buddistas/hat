 ...
 * ФИНАЛЬНАЯ ВЕРСИЯ с правильным алгоритмом нормализации
 * 
 ...
 * 
 * ВСЕ записи из групп дубликатов (включая "оригиналы") перемещаются в конец словаря
...
/**
 * ПРАВИЛЬНАЯ нормализация слова для поиска дубликатов
 * Умный алгоритм, который правильно обрабатывает русские слова
 */
...
function findAndMoveDuplicates() {
  ...
  for (const [normalizedPhrase, entries] of phraseGroups) {
    if (entries.length > 1) {
      ...
      // ВСЕ записи из группы дубликатов перемещаем в конец
      // Это включает и "оригинал", и все дубликаты
