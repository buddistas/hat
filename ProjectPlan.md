...
type Game = {
  ...
  // Перенесенное время (очищается при завершении игры)
  playerCarriedTime: Record<string, number>;
  eventsLogId: string;
  // Реализованная статистика (MVP)
  teamStatsByRound: {
    0: Record<string, number>; // Раунд 1: teamId -> очки
    1: Record<string, number>; // Раунд 2: teamId -> очки  
    2: Record<string, number>; // Раунд 3: teamId -> очки
  };
  playerStats: Record<string, {
    guessed: number;    // Количество угаданных слов
    passed: number;     // Количество пропущенных слов
    totalScore: number; // Общие очки (guessed - passed)
  }>;
};
...
- **Исправления логики переноса времени** (обновлено 21.09.2025):
  ...
  - **Исправление логики записи времени** (обновлено 21.09.2025):
    - Очистка `playerCarriedTime` при завершении игры и начале новой игры
    - Время НЕ переносится между играми (как требовалось)
    - Данные очищаются при завершении партии (предотвращает утечки памяти)
...
### Планируемая аналитика (будущие версии)
- **Детальная статистика**: avgGuessTimeMs, самое медленное/быстрое угадывание
- **Интересные факты**: самое частое PASS‑слово, игрок с наибольшим количеством PASS
- **Персональные рекорды**: сравнение при синхронизации с сервером
- **Секунды переноса**: отображение перенесенного времени между раундами
...
**Проблема**: При угадывании последнего слова в раунде счет не обновлялся корректно, что приводило к расхождению между количеством угаданных слов и отображаемым счетом. Например, при угадывании 20 слов счет показывал только 19 очков.

**Внесенные изменения**:

1. **Анализ проблемы**:
   - Выявлена проблема в функции `wordGuessed()` в `server.js`
   - При угадывании последнего слова функция `nextWord()` сразу завершала раунд, не давая времени на отправку обновленного состояния
   - Функция `broadcastGameState()` не отправляла состояние, если `currentWord === null`

2. **Исправление логики завершения раунда**:
   - Добавлена проверка на окончание слов сразу после удаления угаданного слова
   - Изменен порядок операций: сначала отправка обновленного состояния, затем установка `currentWord = null`
   - Обеспечена корректная отправка финального счета перед завершением раунда
...
4. **Тестирование**:
   ...
   - Созданы автоматизированные тесты для проверки корректности подсчета
   - Подтверждено, что счет всегда соответствует количеству угаданных слов

**Результат**: Проблема с подсчетом результатов полностью решена. Теперь счет всегда точно соответствует количеству угаданных слов, включая последнее слово в игре.
...
**Добавлено**: Подробная статистика результатов игровых сессий с двумя переключаемыми экранами для команд и игроков.

**Внесенные изменения**:

1. **Структура данных для статистики**:
   - Добавлено поле `teamStatsByRound` в `gameState` для хранения очков команд по раундам
   - Добавлено поле `playerStats` для личной статистики игроков (угадано, пропущено, общие очки)
   - Инициализация статистики при старте игры в функции `startGame()`

2. **Сбор статистики в реальном времени**:
   - Обновление `teamStatsByRound` при угадывании/пропуске слов в функциях `wordGuessed()` и `wordPassed()`
   - Обновление `playerStats` для текущего игрока при каждом действии
   - Инициализация статистики для новых раундов в функции `startNextRound()`

3. **Новые экраны результатов**:
   - Заменен простой экран результатов на два переключаемых экрана
   - `teamResultsScreen` - таблица результатов команд по раундам
   - `playerResultsScreen` - таблица личных результатов игроков
   - Навигация между экранами с кнопками "Команды" и "Игроки"

4. **Таблица статистики команд**:
   - Колонки: Команда, Раунд 1, Раунд 2, Раунд 3, Итого
   - Цветовая индикация очков: зеленый (положительные), красный (отрицательные), серый (ноль)
   - Отображение победителя или ничьи

5. **Таблица статистики игроков**:
   - Колонки: Игрок, Команда, Угадано, Пропущено, Очки
   - Сортировка игроков по общим очкам (по убыванию)
   - Цветовая индикация для числовых значений

6. **Стилизация и UX**:
   - Современный дизайн таблиц с тенями и скругленными углами
   - Адаптивный дизайн для мобильных устройств
   - Черный цвет текста для имен игроков и команд для лучшей читаемости
   - Цветная маркировка для очков и числовых значений
   - Убраны скроллбары для лучшего пользовательского опыта

7. **События и интеграция**:
   - Добавлено событие `game_ended` для уведомления клиентов о завершении игры
   - Обновлена обработка WebSocket сообщений для показа новых экранов
   - Интеграция с существующей логикой определения победителя

8. **Функции JavaScript**:
   - `showTeamStats()` - отображение статистики команд
   - `showPlayerStats()` - отображение статистики игроков
   - `determineWinner()` - определение победителя игры
   - `getScoreClass()` - получение CSS класса для цветовой индикации очков

**Результат**: Игроки теперь получают подробную аналитику своих результатов с возможностью переключения между командной и личной статистикой. Таблицы показывают детальную информацию по раундам, что позволяет лучше анализировать игру и планировать стратегию на будущие сессии.
...
3. **Обновление документации**:
   - Обновлен раздел правил игры с указанием очистки времени
   - Обновлен раздел таймера с описанием исправлений
   - Добавлена запись в историю изменений

**Результат**: Логика записи оставшегося времени хода игрока теперь корректна - время не накапливается между играми, очищается при завершении партии, и минимизировано общение с сервером. Предотвращены утечки памяти и обеспечена стабильная работа приложения.