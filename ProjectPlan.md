...
- **Регистр и язык**: все записи в нижнем регистре; язык — русский без транслитераций/англицизмов при наличии устоявшихся аналогов.
- **Формат фраз**: многословные фразы записываются без кавычек; на фронте кавычки не отображаются.
- **Сложность**: не более 25% записей в каждой категории имеют уровень «повышенный», преимущественно для низкочастотных/специализированных терминов (определяется по текущему словарю и здравому смыслу).
...
- **Порядок и сортировка**: новые записи добавляются в конец файла; алфавитная сортировка и группировка по категориям могут быть выполнены позже отдельной задачей.
...
- **Каденция поставок**: добавление ведётся партиями по ~100 слов на итерацию (2 категории × ~50 или 1 категория × ~100) с валидацией после каждой итерации.